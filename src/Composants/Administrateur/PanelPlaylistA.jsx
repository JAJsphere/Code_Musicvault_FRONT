...
export default function PanelPlaylistA() {
  ...
  /* ----------------------------------------------------------- */
  /* --------- PARTIE ADMIN DES PLAYLISTS UTILISATEURS -------- */
  /* ----------------------------------------------------------- */

  // ---------------------------------------------------------------------------- //
  // - FONCTION GET PLAYLISTS USERS -> récup playlists de tous les utilisateurs - //
  // ---------------------------------------------------------------------------- //
  ...
  const getPlaylistUsers = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL_FETCH}/AP_PILLA/PHP_API_BACK/API/PlaylistAdmin.php`,
        {
          method: "GET",
          headers: {
            "Authorization": `Bearer ${token}`
      ...
      if (res.ok) {
        ...
        // Remplacer null/undefined par "Utilisateur inconnu"

        // Pour chaque données renvoyées par l'API (correspondant à une playlist utilisateur)
        data.forEach(p => {

          // Si le login de l'utilisateur est vide / null -> le remplacer par "Utilisateur inconnu" pour éviter d'avoir des cases vides 
          if (!p.loginUtilisateur)
            p.loginUtilisateur = "Utilisateur inconnu";
        });


        // Tri alphabétique par nomUtilisateur
        data.sort((a, b) => a.loginUtilisateur.localeCompare(b.loginUtilisateur));

        // localCompare = Compare deux chaînes de caractères
        // Si a.loginUtilisateur est avant b.loginUtilisateur dans l'alphabet -> renvoie un nombre négatif -> a sera placé avant b
        // Si a.loginUtilisateur est après b.loginUtilisateur dans l'alphabet -> renvoie un nombre positif -> a sera placé après b
        // Si a.loginUtilisateur et b.loginUtilisateur sont identiques -> renvoie 0 -> ordre inchangé

        setPlaylistsUsersAdmin(data); // Je mets les playlists renvoyées par l'API dans le state destiné à l'affichage de l'admin


        // Erreur -> Le serveur répond mais avec une erreur HTTP (404, 500...)
      } else {
        setMessageErreur(`Erreur ${res.status} : impossible de charger les playlists utilisateurs`);
      }

      // Erreur -> Aucune réponse du serveur : problème réseau, timeout... etc
    } catch (error) {
      console.error("Erreur async :", error);
      setMessageErreur("Erreur réseau lors du chargement des données.");
    }
  };
  // -------------------------------------------------- //
  // -------- FIN FONCTION GET PLAYLISTS USERS -------- //
  // -------------------------------------------------- //
  ...
  /* --------------------------------------------------------- */
  /* ------ FONCTION DELETE PLAYLISTS USERS -> ADMIN --------- */
  /* --------------------------------------------------------- */
  const deletePlaylistUsers = async (idPlaylist) => {

    // Si refuse la suppression -> arrêt de la fonction
    if (!window.confirm("Voulez-vous vraiment supprimer cette playlist ?")) {
      return;
    }

    const playlistId = Number(idPlaylist);

    try {
      const res = await fetch(
        `${API_URL_FETCH}/AP_PILLA/PHP_API_BACK/API/PlaylistActions.php?idPlaylist=${playlistId}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${token}`
      ...
      if (res.ok) {
        ...
        if (data.success) {
          ...
          // Affichage admin
          // Renvoie un nouveau tableau avec uniquement les playlists à garder (toutes les playlists sauf celle supprimée)
          if (playlistsUsersAdmin.length > 0) {
            setPlaylistsUsersAdmin(prev =>
              prev.filter(function (p) {

                // Est ce que cette playlist est différente de celle que je veux supprimer ?
                if (Number(p.idPlaylist) !== playlistId) {

                  return true;  // Oui -> On garde 
                } else {
                  return false; // Non -> On ne la garde pas (c'est celle qu'on veut supprimer)
                }
          ...
          // Erreur -> Le serveur a répondu OK mais l'API dit que la suppression a échoué
          setMessagePopup("Erreur lors de la suppression de la playlist.");
        }
  ...
  /* --------------------------------------------------- */
  /* ------ FIN FONCTION DELETE PLAYLISTS -> ADMIN ----- */
  /* --------------------------------------------------- */
  ...
  return (
    <div className="PPA-panel-playlist-admin">
        ...
        <div className="PPA-buttons-actions">
          <div className="PPA-content-wrapper">
            <div className="PPA-buttons-wrapper">
              ...
              {/* Bouton -> Gérer les playlists utilisateurs */}
              <button
                className="PPA-btn-manageUsersPlaylists"
                onClick={() => {
                  setMessagePopup("");
                  setPopupAdminOpen(true);
                  getPlaylistUsers();
                }}
              >
                <FontAwesomeIcon icon={faGear} style={{ marginRight: '8px' }} />
                Gérer les playlists utilisateurs
              </button>
            </div>
          </div>
        </div>
              ...
              playlistsAffichees.map((playlist) => (
                ...
                <div key={playlist.idPlaylist} className="PPA-playlist-card">
                  ...
                  <div className="PPA-playlist-actions">
                    ...
                    <button
                      onClick={() =>
                        navigate(`/Aplaylist/${playlist.idPlaylist}`, { state: { nom: playlist.nom } }) // State -> Permet de transmettre des données à la page de destination sans les mettre dans l'URL. 
      ...
      {/* POPUP ADMIN PLAYLISTS USERS */}
      {popupAdminOpen && (
        <>

          {/* Si on clique sur l'arrière plan pendant qu'on est dans la popup -> fermeture */}
          <div className="PPA-popup-edit-overlay" onClick={() => setPopupAdminOpen(false)}></div>
          <div className="PPA-popup-edit">
            <h3>Playlists utilisateurs</h3>

            <div className="PPA-admin-playlist-list">

              {playlistsUsersAdmin.length > 0 ? (
                playlistsUsersAdmin.map((playlist) => (

                  <div key={playlist.idPlaylist} className="PPA-admin-playlist-row">

                    <div className="PPA-admin-playlist-info">
                      <span className="playlist-name">{playlist.nom}</span>
                      <span className="playlist-user">{playlist.loginUtilisateur}</span>
                    </div>

                    {/* Bouton de suppression d'une playlist de n'importe qui quand on est admin*/}
                    <button
                      className="PPA-admin-btn-delete"
                      onClick={() =>
                        deletePlaylistUsers(playlist.idPlaylist)
                      }
                    >
                      Supprimer
                    </button>
            ...
            <button
              ...
              onClick={() => setPopupAdminOpen(false)}
            >
              Fermer
            </button>
